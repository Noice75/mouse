...
def structure_to_string_method(self):
    ret = [
        f"{self.__class__.__name__} (size: {cts.sizeof(self.__class__)}) instance at 0x{id(self):016X}:"]
...
CreateWindowEx.argtypes = (wts.DWORD, wts.LPCWSTR, wts.LPCWSTR, wts.DWORD, cts.c_int,
                           cts.c_int, cts.c_int, cts.c_int, wts.HWND, wts.HMENU, wts.HINSTANCE, wts.LPVOID)
...
GetRawInputData.argtypes = (
    PRAWINPUT, wts.UINT, wts.LPVOID, wts.PUINT, wts.UINT)
